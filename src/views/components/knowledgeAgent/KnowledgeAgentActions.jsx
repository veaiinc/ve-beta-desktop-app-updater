...
const KnowledgeAgentActions = ({ assistant }) => {
	...
	useEffect(() => {
		if (info?.assistantId) {
			getActionsForKnowledgeAgent(info?.assistantId);
		}
	}, [info?.assistantId]);
	...
	const handleDeleteAction = useCallback(
		async (actionId) => {
			const response = await deleteActionOfKnowledgeAgent(info?.assistantId, actionId);
			if (response) {
				...
				<div className="instructionsListContainer">
					...
					{info?.actionsLoading ? (
						...
						info?.aiActionList?.map((item) => (
							...
							>
								<span className="actionNameContainer">
...
export default memo(KnowledgeAgentActions);
