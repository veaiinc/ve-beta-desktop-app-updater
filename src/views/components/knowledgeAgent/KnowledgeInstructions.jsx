...
const KnowledgeInstructions = ({ instructions }) => {
	...
	const handleToggleChange = useCallback(
		async (instructionId, currentStatus) => {
			try {
				...
				const response = await updateInstructionOfKnowledgeAgent(agentId, instructionId, {
	...
	const handleInstructionAction = useCallback(async () => {
		...
		try {
			if (info?.instructionEditing) {
				...
				const response = await updateInstructionOfKnowledgeAgent(
					agentId,
				...
				if (info?.instructionData?.length < 20) {
					const response = await addInstructionToKnowledgeAgent(
						agentId,
						info?.instructionBody,
					);
					if (response?.[0]) {
	...
	const updateInstructionOfKnowledgeAgentBody = useCallback((field, value) => {
		setInfo((prev) => ({
			...prev,
			instructionBody: {
	...
	const handledeleteInstructionFromKnowledgeAgent = useCallback(async () => {
		setInfo((prev) => ({ ...prev, deletingInstruction: true }));
		const response = await deleteInstructionFromKnowledgeAgent(
			agentId,
			info?.currentInstructionId,
		);
		if (response?.[0]) {
	...
	}, [agentId, info?.currentInstructionId]);
	...
	return (
		<div style={{ width: '100%' }}>
				...
				<div className="instructionsListContainer">
					...
					{info.instructionDataLoading ? (
						...
						info?.instructionData?.map((item) => (
							<div
								key={item?._id}
								className="instructionItem hoverParent"
								onClick={() => handleEditInstruction(item?._id)}
							>
								<span className="hoverChild">
									{item?.title}{' '}
			...
			<InstructionModal
				...
				title={info?.instructionBody?.title}
				instruction={info?.instructionBody?.instruction}
				onTitleChange={(value) => updateInstructionOfKnowledgeAgentBody('title', value)}
				onInstructionChange={(value) =>
					updateInstructionOfKnowledgeAgentBody('instruction', value)
				}
				...
				onDeleteClick={handledeleteInstructionFromKnowledgeAgent}
			/>
		</div>
	);
};

export default memo(KnowledgeInstructions);
