...
const AddToolV2Modal = ({ isOpen, onClose, onToolAdded }) => {
	...
	// Helper: Poll for connected account
	const pollForConnectedAccount = async (appNameSlug, userId) => {
		...
		return new Promise((resolve, reject) => {
			...
			const poll = async () => {
				if (popupRef && popupRef.closed) {
					...
					reject(new Error('Popup closed before connection was completed.'));
					return;
				}
				try {
					const response = await getExistingconnectedAccounts({ tenatUserId: userId });
					const accounts = response?.data?.connected_accounts || [];
					const found = accounts.find(
						(acc) => acc.app && acc.app.name_slug === appNameSlug,
					);
					if (found) {
						clearInterval(pollTimer);
						clearTimeout(timeoutTimer);
		...
		try {
			...
			try {
				await new Promise((resolve, reject) => {
					...
					const poll = async () => {
						try {
							const response = await getExistingconnectedAccounts({
								tenatUserId: userDetailsData?._id,
							});
							const accounts = response?.data?.connected_accounts || [];
							const found = accounts.find(
								(acc) => acc.app && acc.app.name_slug === action.app,
							);
							if (found) {
								clearInterval(pollTimer);
								clearTimeout(timeoutTimer);
								connectedAccount = found;
								resolve();
							}
						} catch (err) {
							// Ignore errors, keep polling
			...
			if (!connectedAccount) throw new Error('Account not connected.');
			...
			(action.configurable_props || []).forEach((prop) => {
				if (prop.type === 'app') {
					props[prop.name] = { authProvisionId: connectedAccount.id };
				} else {
			...
			<div className="actions-modal addtoolv2-modal">
				...
				<div className="actions-modal-inputs">
					...
					{info.isLoading && info.actions.length === 0 ? (
						...
						<InfiniteScroll
							dataLength={info.actions.length}
