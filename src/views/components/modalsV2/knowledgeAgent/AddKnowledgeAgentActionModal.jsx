...
const tooltipStyles = {
	body: { minWidth: 'fit-content', padding: '0', backgroundColor: 'transparent' },
};

const AddKnowledgeAgentActionModal = ({
	isOpen,
	onClose,
	showDelete,
	onDeleteClick,
	...
	const {
		knowledgeAgent: { addActionToKnowledgeAgent, updateActionOfKnowledgeAgent },
	} = useContext(Context);
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (selectedAction) {
				...
				response = await updateActionOfKnowledgeAgent(
					assistantId,
					selectedAction._id,
					payload,
				);
				...
				if (aiActionList?.length < 5) {
					response = await addActionToKnowledgeAgent(assistantId, payload);
				} else {
					onClose();
					message.error('You have reached the maximum limit of 5 actions');
				}
				if (response) {
					onActionAdded(response?.[1]);
...
export default memo(AddKnowledgeAgentActionModal);
