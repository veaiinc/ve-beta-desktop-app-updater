...
const NotchDropLiveKitIntegration = () => {
    ...
    const sendToNotchDrop = useCallback(async (messageData) => {
        try {
            if (window.electronApi && window.electronApi.notchdrop) {
                console.log('📤 SENDING TO NOTCHDROP:', messageData.sender, ':', messageData.content);
    ...
    useEffect(() => {
        ...
        console.log('🎤 NotchDrop LiveKit Integration - Voice Assistant State:', voiceAssistant.state);
        console.log('🎤 NotchDrop LiveKit Integration - Room State:', roomState);
        
        if (voiceAssistant.state === 'disconnected') {
            console.log('🎤 NotchDrop LiveKit Integration - Voice assistant disconnected, skipping transcription processing');
            return;
        }
        ...
        console.log('🎤 NotchDrop LiveKit Integration - Processing local messages, segments count:', localMessages.segments?.length || 0);
        localMessages.segments?.forEach((segment) => {
            console.log('🎤 NotchDrop LiveKit Integration - Processing local segment:', segment);
            const chatMessage = segmentToChatMessage(segment, transcripts.get(segment.id), localParticipant);
        ...
        if (hasNewMessages) {
            ...
            allMessages.forEach((message) => {
                ...
                if (message.isFinal && (!lastSent || lastSent.content !== message.content)) {
                    console.log('📤 Sending FINAL transcription to NotchDrop:', message.sender, ':', message.content);
