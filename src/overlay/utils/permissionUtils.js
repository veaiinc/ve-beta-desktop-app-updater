...
export const checkMicrophonePermission = async () => {
	try {
		// First, try to use Electron's system-level permission checking
		if (window.electronApi?.microphone?.checkPermission) {
			const result = await window.electronApi.microphone.checkPermission();
			if (result.success) {
				return {
					granted: result.hasPermission,
					denied: !result.hasPermission,
					prompt: result.permission === 'not-determined',
					state: result.permission,
				};
			}
		}

		// Fallback: Check if navigator.permissions is available
		...
		// Final fallback: try to access microphone directly
		try {
...
export const requestMicrophonePermission = async (constraints = { audio: true }) => {
	try {
		...
		// First, try to use Electron's system-level permission request
		if (window.electronApi?.microphone?.requestPermission) {
			const result = await window.electronApi.microphone.requestPermission();
			if (result.success && result.granted) {
				console.log('Microphone permission granted via Electron API');
				return { success: true, stream: null };
			} else if (result.success && !result.granted) {
				console.log('Microphone permission denied via Electron API');
				return {
					success: false,
					error: 'NotAllowedError',
					message:
						'Microphone access denied. Please allow microphone access in your system settings and try again.',
				};
			}
		}
...
export const requestAndTestMicrophoneAccess = async () => {
	...
	// First try Electron's system-level permission check
	if (window.electronApi?.microphone?.checkPermission) {
		try {
			const result = await window.electronApi.microphone.checkPermission();
			if (result.success) {
				currentPermissionState = result.permission;
				console.log('Current permission state (Electron):', result.permission);
			}
		} catch (e) {
			console.log('Electron permission query failed, trying browser fallback:', e.message);
		}
	}

	// Fallback to browser permissions
	if (currentPermissionState === 'unknown' && navigator.permissions) {
		try {
			const permission = await navigator.permissions.query({ name: 'microphone' });
			currentPermissionState = permission.state;
			console.log('Current permission state (Browser):', permission.state);
		} catch (e) {
	...
	if (currentPermissionState === 'denied') {
		return {
			...
			error: {
				...
				message:
					'Microphone permission was previously denied. Please enable it manually in your browser or system settings.',
			},
	...
	try {
		...
		if (stream.getAudioTracks().length > 0) {
			try {
				...
				console.log(
					'Audio context test failed, but microphone access successful:',
					audioError,
				);
			}
		}
		...
		if (error.name === 'NotAllowedError') {
			...
			troubleshootingTips.push(
				'🔧 You may need to restart the app after granting system permissions',
			);
		} else if (error.name === 'NotFoundError') {
...
export const requestAndTestClipboardAccess = async () => {
	...
	if (navigator.permissions) {
		try {
			...
			console.log(
				'Clipboard permission query failed, will attempt direct access:',
				e.message,
			);
		}
	}
	...
	if (currentPermissionState === 'denied') {
		return {
			...
			error: {
				name: 'PermissionPreviouslyDenied',
				message:
					'Clipboard permission was previously denied. Please enable it manually in your browser settings.',
			},
			needsPermission: true,
			needsManualEnable: true,
		};
	...
	try {
		...
		if (error.name === 'NotAllowedError') {
			...
			troubleshootingTips.push(
				'🔧 Look for a clipboard icon in your browser\'s address bar and click "Allow"',
			);
...
export const testLiveKitCompatibility = async () => {
	...
	try {
		...
		if (audioTrack) {
			...
			console.log(
				allMatch
					? '✅ All LiveKit constraints satisfied'
					: '⚠️ Some constraints not fully satisfied',
			);
		}
