...
class AudioStorageService {
	...
	async getAudio(meetingId) {
		try {
			...
			try {
				...
				console.error('AudioStorageService: Raw data that failed to parse:', JSON.stringify(metadataString));
				console.error('AudioStorageService: Character at position 306:', metadataString.charAt(306));
				console.error('AudioStorageService: Characters around position 306:', metadataString.substring(300, 310));
			...
			const audioBlob = new Blob([audioResult.data], { type: metadata.mimeType || 'audio/webm' });
	...
	async getAudioMetadata(meetingId) {
		try {
			...
			console.log('AudioStorageService: Raw metadata length (getMetadata):', metadataString.length);
			...
			try {
				...
				console.error('AudioStorageService: Raw data that failed to parse (getMetadata):', JSON.stringify(metadataString));
				throw new Error(`Failed to parse metadata JSON: ${parseError.message}`);
			}
	...
	async hasAudio(meetingId) {
		try {
			...
			console.log('AudioStorageService: Audio exists:', audioExists, 'Metadata exists:', metadataExists);
