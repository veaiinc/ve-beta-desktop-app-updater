...
function checkBuildArtifacts() {
    ...
    artifacts.forEach(artifact => {
        ...
        if (fs.existsSync(artifactPath)) {
            ...
            log('green', `✅ Found build artifact: ${artifact} (${(stats.size / 1024).toFixed(1)}KB)`);
        } else {
...
function main() {
    ...
    const checks = [
        { name: 'Platform check', fn: () => process.platform === 'darwin' || (log('red', '❌ NotchDrop addon only supports macOS'), false) },
    ...
    if (allPassed) {
        log('green', `${colors.bold}✅ All validation checks passed! NotchDrop addon is ready to build.${colors.reset}`);
        process.exit(0);
    } else {
        log('red', `${colors.bold}❌ Some validation checks failed. Please fix the issues above before building.${colors.reset}`);
        process.exit(1);
    }
}
