...
function installPythonDeps(pythonCmd) {
	...
	try {
		...
		try {
			...
			const pyaudioAlternatives = ['pyaudio==0.2.14', 'pyaudio --only-binary=all', 'PyAudio'];
			...
			if (partialSuccess && !pyaudioInstalled) {
				...
				console.log(
					`   cd electron/wakeWord && ${pythonCmd} -m pip install -r requirements.txt`,
				);
				return false;
			}
...
async function setupHeyVe() {
	try {
		...
		if (modelCheck.missing.length > 0) {
			console.log(
				`❌ Missing ONNX models (${modelCheck.missing.length}/${modelCheck.total}):`,
			);
