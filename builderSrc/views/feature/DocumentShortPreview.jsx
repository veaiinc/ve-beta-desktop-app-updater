...
class DocumentShortPreview extends NewProposals {
	constructor(props) {
		...
		this.state = {
			...
			isLoading: true,
			activeModuleId: this.props.doc.firstModule[0]._id,
	...
	componentDidMount = async () => {
		...
		const module = this.props.doc.firstModule[0]._id;
		const isPublic = this.state.isPublicModule;
		const restrictClick = this.state.restrictClick;
		const moduleType = this.state.moduleType;
		...
		const workflow = this.state.isWorkflow;
		this.setState({
			isPublicModule: isPublic,
			isWorkflow: workflow,
		});
	...
	// module = this.props.doc.firstModule[0];
							...
							>
								...
								>
									<Builder
										...
										deleteFQBlock={(blockID, sectionID) => console.log()}
										handleOpenSideBar={() => {}}
										activeFontColor={this.state.activeFontColor}
										actionType={this.state.actionType}
										actionValue={this.state.actionValue}
										handleHSelection={(e, activeTextBlock) => console.log()}
										crop={this.state.crop}
										zoom={this.state.zoom}
										sections={this.state.moduleData.sections}
										...
										handleSetBlockContent={(content, sectionID, blockID) =>
											console.log()
										}
										...
										variables={this.state.moduleData.variables}
										tables={this.state.moduleData.tables || []}
										tablesValuesforClient={this.state.moduleData.tables || []}
										...
										addFormQuestion={(sectionID, blockOrder) => console.log()}
										setActiveFormQuestion={(e, blockID) =>
											this.setState({
												activeSubBlockType: 'q',
												activeFormQuestion: blockID,
												showSideBar: this.state.preview ? false : true,
											})
										}
...
export default withRouter(DocumentShortPreview);
