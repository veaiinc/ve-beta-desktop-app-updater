...
const DuplicateLeadModal = ({ open, closeModal, duplicateLeadFunc }) => {
	const [info, setInfo] = useState({
		loader: false,
	});

	const duplicateOnClick = useCallback(async () => {
		try {
			setInfo((prev) => ({ ...prev, loader: true }));
			await duplicateLeadFunc();
		} finally {
			modyfyClose();
		}
	}, []);
		...
		>
			<div className="deleteLeadModalParentContainer">
				...
				<div className="deleteLeadFooterContainer">
					...
					<div className="deleteBtn" onClick={duplicateOnClick}>
						{info?.loader ? 'Duplicating... ' : 'Duplicate'}
...
export default memo(DuplicateLeadModal);
