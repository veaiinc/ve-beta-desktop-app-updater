...
const EventComponent = () => {
	...
	const changeSliderHandler = useCallback(
		(handleType, handleValue, screenType) => {
			...
			if (handleType === 'eventTitle') {
				if (newTheme?.isDesktopMobileFontLinked) {
					updateTheme.fonts.event.eventTitle.fontSize = `${handleValue}px`;
					updateTheme.mobileFonts.event.eventTitle.fontSize = `${responsiveFontSize[handleValue]}px`;
				} else if (screenType === 'desktop') {
					updateTheme.fonts.event.eventTitle.fontSize = `${handleValue}px`;
					updateTheme.fonts.event.eventTitle.fontSize = `${handleValue}px`;
				} else if (screenType === 'mobile') {
					updateTheme.mobileFonts.event.eventTitle.fontSize = `${handleValue}px`;
					updateTheme.mobileFonts.event.eventTitle.fontSize = `${handleValue}px`;
				}
			} else if (handleType === 'letterSpacing') {
				updateTheme.fonts.event.eventTitle[handleType] = `${handleValue}px`;
				updateTheme.fonts.event.eventTitle[handleType] = `${handleValue}px`;
			} else if (handleType === 'lineHeight') {
				updateTheme.fonts.event.eventTitle[handleType] = handleValue;
				updateTheme.fonts.event.eventTitle[handleType] = handleValue;
			}
	...
	const handleSelectHandler = (type, value) => {
		...
		let updateTheme = _.cloneDeep(newTheme);
		let updatedSections = _.cloneDeep(sections);
		const { event } = updateTheme.fonts;

		if (type === 'fontFamily') {
			let currentFont = _.find(fontsArray, { _id: value });
			updateState.selectedFont = currentFont;

			updateTheme.fonts.event.eventTitle = {
				...event.eventTitle,
			...
			updateTheme.fonts.event.eventTitle[type] = value;
		}
		...
		<div className="FontheadingContainer">
				...
				<div className="gridBox fontFamilyContainer">
					...
					<div className="fontFamilyOptionsContainer">
						<Select
							...
							value={newTheme?.fonts?.event?.eventTitle?.activeFontID}
				...
				<div className="gridBox fontFamilyContainer">
					...
					<div className="fontFamilyOptionsContainer">
						<Select
							...
							value={newTheme?.fonts?.event?.eventTitle?.textTransform || 'none'}
				...
				<div className="gridBox fontWeightContainer">
					...
					<div className="fontFamilyOptionsContainer">
						<Select
							...
							value={newTheme?.fonts?.event?.eventTitle?.activeVariant}
				...
				<div className="gridBox fontFamilyContainer">
					...
					<div className="sliderContainer">
						<Slider
							...
							step={0.1}
							max={5}
							value={getNumberFromPx(fontStyles?.event?.eventTitle?.lineHeight)}
				...
				<div className="gridBox fontWeightContainer">
					...
					<div className="sliderContainer">
						<Slider
							min={0}
							max={5}
							value={getNumberFromPx(fontStyles?.event?.eventTitle?.letterSpacing)}
			...
			{newTheme?.isDesktopMobileFontLinked ? (
					...
					<div className="gridBox fontFamilyContainer">
						...
						<div className="sliderContainer">
							<Slider
								min={0}
								max={100}
								value={getNumberFromPx(fontStyles?.event?.eventTitle?.fontSize)}
								onChange={(value) => changeSliderHandler('eventTitle', value)}
							/>
							<span className="sliderValue">
								{getNumberFromPx(fontStyles?.event?.eventTitle?.fontSize)}
							</span>
							<span className="sliderValue">
								{getNumberFromPx(mobileFontStyles?.event?.eventTitle?.fontSize)}
							</span>
						</div>
					</div>
				</div>
				...
				>
					...
					<div className="gridBox">
						<div className="gridBox_title"> Action Block Title</div>
						...
						<div className="gridBox_input">
							<input
								...
								value={getNumberFromPx(
									mobileFontStyles?.event?.eventTitle?.fontSize,
								)}
								max={500}
								min={0}
								onChange={(e) =>
