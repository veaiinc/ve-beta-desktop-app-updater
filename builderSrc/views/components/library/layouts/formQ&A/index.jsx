...
class Form extends Component {
	...
	handleDeleteSection = (e) => {
		this.props.deleteSection(this.state.sections[0]._id);
	};
	handleBlock = (e) => {
	...
	render() {
		...
		return (
			<div
				...
				onClick={(e) =>
					this.setState({ showBlockActions: true }, () => {
						this.props.handleSideBar(e, this.state.sections[0]._id);
					})
				}
			>
				...
				{_.map(sectionsArr, (block, key) => {
					return (
						<FormElement
							...
							setActiveTheme={this.state?.style?.theme || this.state.setActiveTheme}
							isTheme={this.state?.style?.isTheme || this.state.isTheme}
				...
				<div className="form-button">
					<a
						style={{
							backgroundColor: this.state?.style?.isTheme
								? this.state?.style?.theme?.button
								: '#333',
							color: this.state?.style?.isTheme
								? this.state?.style?.theme?.buttonText
								: '#fff',
							borderRadius: '50px',
							width: '100%',
							padding: '12px 15px',
						...
						onClick={(e) => {
							if (this.props.client && !this.state?.submitFormLoading) {
								this.props.submitForm(e, this.props?._id);
							}
						}}
					>
