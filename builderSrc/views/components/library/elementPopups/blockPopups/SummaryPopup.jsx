...
export default class SummaryPopup extends Images {
    ...
    handleVideoProps = (type, value) => {
        ...
        this.setState({ activeComponent: newComponent }, () => {
            this.props?.setActiveSection(newComponent, true, false);
        });
    };
                        ...
                        <div className="element_image">
                            <div className="element_pasteURL">
                                        ...
                                        <>
                                            ...
                                            >
                                                ...
                                                {(() => {
                                                    ...
                                                    const searchFiltered = this.props?.fonts?.filter((font) => {
                                                        if (!this.state.searchFont) return true;
                                                        return font.fontName?.toLowerCase().includes(this.state.searchFont.toLowerCase());
                                                    }) || [];

                                                    // Group fonts by their 'group' property
                                                    const groupedFonts = _.groupBy(searchFiltered, 'group');

                                                    return Object.entries(groupedFonts).map(([groupName, groupFonts]) => (
                                                        <div key={groupName} className="font-group">
                                                            <div className="group-title">{groupName}</div>
                                                            <div className="group-fonts">
                                                                {_.orderBy(
                                                                    _.uniqBy(groupFonts, (font) => font.fontName?.toLowerCase()),
                                                                    [(font) => font.fontName?.toLowerCase()],
                                                                    ['asc']
                                                                ).map((font, k) => (
                                                                    <div
                                                                        key={`${groupName}-${k}`}
                                                                        className={`font-item ${this.state.activeFont === font.value ? 'active' : ''}`}

                                                                        onClick={(e) => this.handleSummaryStyles(
                                                                            e,
                                                                            'summaryPrimaryFontFamily',
                                                                            font.value
                                                                        )}
                                                                    >
                                                                        <p
                                                                        ...
                                                                        {this.state.activeFont === font.value && <ActiveTick />}
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        </div>
                                        ...
                                        <div className="popup-range-div">
                                            ...
                                            >
                                                <input
                                                    ...
                                                    step={1}
                                                    value={
                                                        this.state.activeFontSize
                                                    }
                                                    onChange={(e) =>
                                                        this.handleSummaryStyles(
                                                            e,
                                            ...
                                            >
                                                <input
                                                    ...
                                                    type="number"
                                                    value={this.state.activeFontSize}
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                        const finalValue = !isNaN(value) && value >= 8 ? value : 8;
                                                        this.handleSummaryStyles(
                                                            e,
                                                            'summaryPrimaryFontSize',
                                                            finalValue
                                ...
                                <div className="padding-options-wrapper">
                                    ...
                                    <div className="padding-options-item">
                                        <div className={`padding-one ${!_.has(this.props?.activeComponent?.style, 'paddingHorizontal') || this.props?.activeComponent?.style?.paddingHorizontal === 0 ? 'active' : ''}`} onClick={() => this.handleSummaryStyles(null, 'paddingHorizontal', 0)}>null</div>
                                        <div className={`padding-one ${this.props?.activeComponent?.style?.paddingHorizontal == 1 ? 'active' : ''}`} onClick={() => this.handleSummaryStyles(null, 'paddingHorizontal', 1)}>S</div>
                                        ...
                                        <div className={`padding-one ${this.props?.activeComponent?.style?.paddingHorizontal == 3 ? 'active' : ''}`} onClick={() => this.handleSummaryStyles(null, 'paddingHorizontal', 3)}>L</div>
                                        <div className={`padding-one ${this.props?.activeComponent?.style?.paddingHorizontal == 4 ? 'active' : ''}`} onClick={() => this.handleSummaryStyles(null, 'paddingHorizontal', 4)}>XL</div>
                                    </div>
                                </div>
