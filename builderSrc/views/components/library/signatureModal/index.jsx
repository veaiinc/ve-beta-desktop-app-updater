...
class SignatureModal extends Component {
	...
	setSignature = async () => {
		...
		if (this.state.signatureType === 'text') {
			...
			await this.props.handleSignature({
				signature: {
					type: 'text',
			...
			await this.props.handleUploadSignature(file);
			if (this.props.signatureUploaded) {
				this.props.handleClose();
				this.state.handleContractNaviagtion();
			}
		...
		return (
			<div className="sign-block-wrapper">
				{window.innerWidth > 768 ? (
									...
									<>
										...
										<div className="terms-and-submit">
											<div className="terms">
												...
												<p>
													I agree that my electronic signature is as valid
													a legally bindings handwritten signature.
												</p>
											</div>
											<button
												className={`submit ${
					...
					<MobileSignatureModal {...this.props} />
				)}
			</div>
		);
	}
