...
// across both REST and GraphQL services in builderSrc
let builderSrcSharedRefreshTokenPromise = null;

const performBuilderSrcSharedTokenRefresh = async () => {
...
/**
 * Shared token refresh function for builderSrc that ensures only one refresh token call happens
 * across all builderSrc services (REST and GraphQL) at any given time
 * @returns {Promise<{success: boolean, accessToken?: string, refreshTokenResponse?: any, status?: number}>}
 */
export const getBuilderSrcSharedRefreshToken = async () => {
	// If there's already a refresh token call in progress, wait for it
	if (builderSrcSharedRefreshTokenPromise) {
		return await builderSrcSharedRefreshTokenPromise;
	}

	// Start a new refresh token call
	builderSrcSharedRefreshTokenPromise = performBuilderSrcSharedTokenRefresh();

	try {
		return await builderSrcSharedRefreshTokenPromise;
	} finally {
		// Clear the promise so future calls can start a new refresh if needed
		builderSrcSharedRefreshTokenPromise = null;
	}
};

export default getBuilderSrcSharedRefreshToken;
