...
class NewProposals extends Component {
	...
	getIndividualWorkflowInfo = async (query, variables) => {
		...
		// Validate workflowID
		if (!workflowID) {
			console.error('workflowID is undefined in getIndividualWorkflowInfo');
			return [false, { message: 'Invalid workflowID' }];
		}
		...
		try {
			...
			if (response[0] === true) {
				...
				this.setState(
					{
						...
						activeModule: modules?.find(
							(module) => module.templateId === template?._id,
						),
						workflow_id,
						template_ID:
							response?.[1]?.data?.workflowInfo?.template
								?.workflowTemplateDetails?.[0]?._id,
				...
				return [true, response[1].data.workflowInfo];
			} else {
				console.error('GraphQL query failed:', response[1]?.errors || 'Unknown error');
				return [false, response[1]?.errors || { message: 'Query failed' }];
			}
		} catch (error) {
			console.error('Error in getIndividualWorkflowInfo:', error);
			return [false, { message: 'Internal error', error }];
		}
	};
	...
	createNavbarFunction = async (query, variables) => {
		...
		if (response[0] === true) {
			...
			// return response[1].data.createNavbar;
		}
	};

	handleMessage = (event) => {
		if (event.data.type === 'SCROLL_TO_ELEMENT' && event.data.fromSidebar) {
			this.scrollAndHighlightElement(event.data.id);
		} else if (event.data.type === 'VARIABLE_UPDATE') {
			this.handleVariableUpdate(event.data.id, event.data.value);
		} else if (event.data.type === 'SERVICE_BLOCK_UPDATE') {
			this.handleServiceBlockUpdate(event.data.blockId, event.data.data);
		}
	};

	scrollAndHighlightElement = (elementId) => {
		...
		if (element) {
			...
			document.querySelectorAll('.highlight-animation').forEach((el) => {
	...
	handleVariableUpdate = async (id, value, type = 'variable') => {
		const json = {
			id,
			value,
			type,
		...
		await this.updateVariables(json, this.state.activeModuleId, id);

		if (window.frames['preview-frame']) {
	...
	handleServiceBlockUpdate = async (blockId, data) => {
		const json = {
			blockId,
			data,
		};

		await this.putSections(json);

		if (window.frames['preview-frame']) {
			window.frames['preview-frame'].postMessage(
				{
					type: 'SERVICE_BLOCK_UPDATE',
					blockId,
					data,
				},
				'*',
