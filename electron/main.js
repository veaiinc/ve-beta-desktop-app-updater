...
// Import NotchDrop service - DISABLED
...
const canPerformBackgroundUpdate = () => {
	...
	const idleDurationMs = 0; // idleTracker disabled
	const isIdleEnough = false; // idleTracker disabled
	const inMeeting = false; // meetingState disabled
...
async function showNotification(title, body) {
	...
	// Send notification to SwiftUI NotchDrop - DISABLED
...
// 			} catch (mediaError) {
// 				log.warn(
// 					'⚠️ Privacy_Media failed, trying Photos as fallback:',
// 					mediaError.message,
// 				);
...
function createMenuBar() {
	...
	const template = [
		...
		// 			// updateMenuBarState(); // NotchDrop disabled
		// 		}
		// 	}
		// } catch (error) {
		// 	log.error(
	...
	// NotchDrop menu state updates disabled
	// setTimeout(() => {
	// 	updateMenuBarState();
	// }, 2000); // Wait for NotchDrop service to initialize
}

// NotchDrop menu updates disabled
// function setupNotchDropMenuUpdates() {
// 	// NotchDrop disabled - skipping menu updates
// }

// NotchDrop menu state updates disabled
// function updateMenuBarState() {
// 	// NotchDrop disabled - skipping menu state updates
// }
...
function createWindow(restoreState = false) {
	...
	const defaultBounds = { width: 1366, height: 768, x: 100, y: 100 };
	const windowBounds =
	...
	// Force dock icon to show immediately after window creation
	if (process.platform === 'darwin') {
		app.dock.show();
	...
	// Send the same message to Swift UI if NotchDrop service is available - DISABLED
	// if (notchDropService && notchDropService.isInitialized) {
	// 	try {
...
<body>
	...
	mainWindow.once('ready-to-show', () => {
		// Show the main window on startup (removed minimize)
		log.info('🎯 Window ready-to-show event fired');
		mainWindow.show();
		mainWindow.focus();
		mainWindow.moveTop();
		mainWindow.setAlwaysOnTop(false); // Ensure it's not always on top
...
app.whenReady().then(async () => {
	...
	// Initialize idle tracker and meeting state listeners for background updates - DISABLED
	// try {
	...
	// store = createStore();
	// meetingState.setStore(store); // meetingState disabled
	// bridge = createBridge(store);

	// Apply initial desktop settings - show dock icon for main app
	if (process.platform === 'darwin') {
		// Set activation policy to 'regular' to ensure app shows in dock
		app.setActivationPolicy('regular');
		app.dock.show();
		log.info('🍎 Dock icon visible - main app mode');
	}
...
ipcMain.handle('get-window-info', (event) => {
	try {
		...
		if (window === mainWindow) {
			...
			windowType = 'overlay';
			windowId = 3;
		} else if (dynamicIslandHelper && window === dynamicIslandHelper.getDynamicIslandWindow()) {
...
// Pre-create overlay window (non-blocking)
if (windowHelper?.preCreateOverlayWindow) {
	windowHelper.preCreateOverlayWindow().catch((error) => {
...
// NotchDrop initialization disabled
...
// NotchDrop status change listener disabled
...
// NotchDrop IPC handlers disabled
...
// 	// NotchDrop disabled
// 	return { success: false, error: 'NotchDrop service disabled' };
// });

// ipcMain.handle('notchdrop-is-visible', async () => {
// 	// NotchDrop disabled
// 	return { success: false, visible: false, error: 'NotchDrop service disabled' };
// });
...
// Menu update handler
// NotchDrop menu update handler disabled
...
app.on('will-quit', (event) => {
	try {
		...
		// Clean up NotchDrop service - DISABLED
		// if (notchDropService) {
		// 	notchDropService.cleanup();
		// }
