...
app.whenReady().then(async () => {
	...
	if (process.platform === 'darwin') {
		app.dock.hide();
		log.info('🍎 Dock icon hidden on startup - running in background mode by default');
	}
	...
	setTimeout(async () => {
		try {
			...
			if (!isAuthenticated) {
				log.info('👤 User not authenticated - permission overlay will show on login');
			} else {
				// If already authenticated, check permissions and show overlay if needed
				const permissionsGranted = await checkAllPermissions();
				if (!permissionsGranted.allGranted) {
					log.info('ℹ️ Authenticated user - showing optional permission overlay');
					windowHelper?.showPermissionWindow();
				} else {
	...
	ipcMain.handle('askAI-move-to', async (event, { x, y }) => {
		try {
			const askAIWindow = windowHelper?.getAskAIWindow();
			if (askAIWindow && !askAIWindow.isDestroyed()) {
				const clamped = windowHelper?.constrainAskAIWindowPosition({
					x,
					y,
					...askAIWindow.getBounds(),
				}) || { x, y };
